...
The user widened the scroll window to 6–8 months; the default is now seven months per viewport. The GitHub daily workflow completed successfully, refreshing 200 citation records before the shared provider rate limit. It preserves the other records and resumes with unchecked papers. The older Codex schedule remains active pending explicit permission to pause it.
