...
## Age-dependent weighting and bounded selection — policy version 4

The user requested almost no weight on a new paper's own citations, a linear shift toward citations as it matures, different category thresholds, and roughly 800 visible papers. The implemented weights are 0% citations at release, 25% after six months, 50% after a year, and 100% at two years. The complementary weight uses the strongest sourced company, curated author, or publication-linked author h-index signal. Both signals use a 0–100 scale; their exact formula is in README.md. Weights are not reallocated when evidence is missing.

The updater retrieves author identities via paper records and author metrics by ID, with conservative exact-name checks against arXiv authors. It stores profiles once, refreshes their metrics weekly, and recomputes scores daily. Historical selections, foundations, and explicit recent editorial learning selections remain available. Other qualifying papers are selected using a base cutoff recalibrated toward 800 visible papers. Category factors are systems 0.70, robotics 0.90, benchmarks 0.95, text 1.00, vision 1.10. Scores are not changed to fit that target, and missing/zero evidence is not used to pad the count.

The September 8 snapshot has 799 visible papers, including 244 from the last two years and 45 recent systems papers. Total visible category counts are 165 systems, 320 text, 171 vision, 57 robotics, and 86 benchmarks. The full catalog still contains 2,238 papers. The graph preserves domain colors, the unlabeled top edge, and the seven-month scroll scale; hover details now show both weights and their evidence.
...
## Reliable sources and monthly density — policy version 6

The user requested a fix for provider throttling and the illegible recent end of the graph. Replace the arXiv export search endpoint with arXiv's metadata deposits in DataCite, retaining original submission dates and an independent resumable registration window. Add exact-identity, title-verified OpenAlex observations and citation edges without combining provider counts. Reuse fresh citation evidence, obtain author identities in the citation batch first, pace requests centrally, and persist provider cooldowns after rate limits.

Keep the existing linear score equation and category factors. Raise category-adjusted admission thresholds in crowded months to enforce a 40-paper monthly ceiling, with learning anchors exempt. Spread nearby bubbles within their calendar month and category lane while preserving the seven-month viewport. The initial recovery added 489 tracked papers and reduced September from 119 visible papers to 40, with 631 visible papers overall. All excluded papers remain in the catalog for subsequent rescoring.
